...
function mainPage() {
    ...
    // 2-1-1. 메인페이지 스크롤
    // 메인비주얼 휠 이벤트
    document.querySelector('.main_visual').addEventListener('wheel', function (event) {
    ...
    document.querySelector('.main_visual').addEventListener('click', function (event) {
        event.preventDefault();
...
function showPost() {
    ...
    nickName.forEach(function (name) {
        name.textContent = '[ ' + name.closest('.post').querySelector('img').getAttribute('data-nick-name') + ' ]';
    });

    titleH3.forEach(function (h3) {
        h3.textContent = h3.closest('.post').querySelector('img').getAttribute('alt');
    });
...
function bestPage() {
    ...
    document.querySelector('.best_view').addEventListener('click', () => {
        ...
        document.querySelector('.best_customizing').classList.remove('click_second');
        images.forEach((img) => {
...
function viewInfo() {
    const continentButtons = document.querySelectorAll('input[name="continent_btn"]'); // continet 버튼
    ...
    closeBtn.addEventListener('click', function () {
        ...
        // 지도 원상복귀
        bg.style.background = 'url(../../img/photo/view/maps/continent/main_map.jpg) no-repeat center / cover';
    ...
    continentButtons.forEach((button) => {
        button.addEventListener('change', () => {
            ...
            bg.style.background =
                'url(../../img/photo/view/maps/continent/background/' + labelClass + '.png) no-repeat center / cover';
    ...
    const radioButtons = document.querySelectorAll('.detail_btns input[type="radio"]');
    ...
    function setInitialValue() {
        // 체크된 라디오 버튼 찾기
        const checkedRadio = document.querySelector('.detail_btns input[type="radio"]:checked');
    ...
    function noticeBoardEvent() {
        ...
        serviceBoard.addEventListener('click', function () {
            setTimeout(() => {
                let textTypes = document.querySelectorAll('.type');
                textTypes.forEach((element) => {
                    if (element.innerText === '서비스/기능' || element.innerText === '종류') {
                        element.parentElement.style.display = '';
                    } else {
                        element.parentElement.style.display = 'none';
                    }
    ...
    function noticeColor() {
        setTimeout(() => {
            ...
            textTypes.forEach((element) => {
                ...
                } else if (element.innerText === '서비스/기능' || element.innerText === '이벤트') {
...
function customizingInfo() {
            ...
            radio.addEventListener('change', () => {
                if (radio.checked) {
                    ...
                    if (radio.value.includes('specialist')) {
                        dummy.setAttribute('src', '../../img/photo/customizing/customizing_info/avartar_sp.png');
                        characterType = '캐릭터(여)';
                    } else if (radio.value.includes('female')) {
                        dummy.setAttribute('src', '../../img/photo/customizing/customizing_info/avartar_f.png');
                        characterType = '캐릭터(여)';
                    } else if (radio.value.includes('male')) {
                        dummy.setAttribute('src', '../../img/photo/customizing/customizing_info/avartar_m.png');
                        characterType = '캐릭터(남)';
                    }

                    avartarFrame.forEach((frame) => {
                        const jobData = frame.querySelector('img').getAttribute('data-job');
                        const jobKeyword = frame.querySelector('img').getAttribute('data-job-keyword');

                        // 조건에 맞는 경우에만 display block, 나머지는 none
                        if (jobData === characterType || jobKeyword === radio.getAttribute('id')) {
    ...
    let totalPages;
    let pageNum = false;
    let filteredFrames = []; // 필터링된 frame 목록
    ...
    // reset 버튼 클릭 시 모든 radio와 checkbox 체크 해제
    document.querySelector('.reset').addEventListener('click', function () {
        // 모든 radio 버튼 체크 해제
        jobRadioBtn.forEach((radio) => {
            if (radio.getAttribute('id') == 'fighter_male') {
                radio.checked = true;
            } else {
                radio.checked = false;
            }
        ...
        // 체크 상태에 따라 표시되는 div 수정
        const partDivs = document.querySelectorAll('.checking .part'); // partDivs 정의
        partDivs.forEach((div) => {
            div.parentNode.removeChild(div); // part div 제거
        });
        job.innerText = '#전사(남)';

        // 이미지와 텍스트 초기화
        dummy.setAttribute('src', '../../img/photo/customizing/customizing_info/avartar_m.png'); // 기본 이미지로 초기화
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (pathName.includes('best_view.html')) {
        ...
        if (pathName.includes('view_info.html')) {
            ...
            community();
            sortByFnc();
        } else if (pathName.includes('view_forum.html') || pathName.includes('customizing_forum.html')) {
            viewForum();
            sortByFnc();
            pageNumber();
